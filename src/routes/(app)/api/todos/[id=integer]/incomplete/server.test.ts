...
//
describe("api/todos/[id]/incomplete", () => {
  // ------------------------
  // PUT METHOD
  // ------------------------
  describe("PUT", () => {
    it("更新成功", async () => {
      ...
      const todoRecord = await prisma.todos.create({
        data: { body: "abc", isCompleted: true },
      });
      // Act
      ...
      // Assert
      expect(results.isCompleted).toStrictEqual(false);
      const updatedRecord = await prisma.todos.findFirst({
        where: { id: todoRecord.id },
      });
      expect(updatedRecord?.isCompleted).toStrictEqual(false);
    });
